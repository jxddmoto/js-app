...
describe('Starting tests for healthcare-app', function() {
  ...
  it('navbar-brand should be healthcare-app',() => {
    ...
    it('Sample component should be loadable',() => {
      page.navigateTo('/Sample');
    ...
    it('Sample table should have 9 columns',() => {
      page.navigateTo('/Sample');
    ...
    it('Business table should have 6 columns',() => {
      page.navigateTo('/Business');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(6); // Addition of 1 for 'Action' column
      });
    ...
    it('CreateSample component should be loadable',() => {
      page.navigateTo('/CreateSample');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('CreateSample');
      });
    });
  
    it('GetSample component should be loadable',() => {
      page.navigateTo('/GetSample');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('GetSample');
      });
    });
  
    it('UpdateSample component should be loadable',() => {
      page.navigateTo('/UpdateSample');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('UpdateSample');
      });
    });
